...
$con = con();
 ...
 $sql="SELECT *, ingreso_entradas.id AS natalie FROM ingreso_entradas
 ...
 // var_dump(mysqli_query($conexion, $sql));
  ...
  while($row = mysqli_fetch_array($result)) {
    ...
    $json[] = array(
      ...
      'cate'=> $row['categoria'],
      'botones'=>'<td>
            <button type="button" id="ver" class="btn btn-info rounded-pill vere-item" id-item-vere="'.$row['natalie'].'" title="Ver"><i 
            class="far fa-eye" data-coreui-toggle="modal" data-coreui-target="#modalVer"></i></button>
            <button type="button" id="ver" class="btn btn-warning rounded-pill" title="Editar">
            <i class="far fa-edit"></i></button>
            <a  href="../dist/AsignaciondeActivo.php">
